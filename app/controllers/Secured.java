...
public class Secured extends Security.Authenticator {
   public static  UserInfo info1 = new UserInfo();
    /**
    ...
    public static int isServiceProvider(Context ctx) {

        if(isLoggedIn(ctx)) {

            if (UserInfoDB.getUser(getUser(ctx)).getRole().equals("Service Provider")) {
                System.out.println("it is service provider");
                return 1;
            }
            if(UserInfoDB.getUser(getUser(ctx)).getRole().equals("Service User"))
                return 2;

            if(UserInfoDB.getUser(getUser(ctx)).getRole().equals("admin"))
                return 3;
        }
        return 0;
    }
